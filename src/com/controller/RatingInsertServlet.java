...
public class RatingInsertServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ValidationUtils.isEmpty(description)){
			...
			rating.setRating(description);
		}
		

		rating.setDateOfCreation(dateOfCreation);
		
		if(isError){
			System.out.println("validation error");
			request.getRequestDispatcher("ratingInsert.jsp").forward(request, response);	
		}else{
						
			if(new RatingDao().insert(rating)){
